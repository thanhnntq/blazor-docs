...
You can validate the TreeList with any validator that uses the `EditContext`. To change the default validator, add a `<ValidatorTemplate>` tag in `<TreeListValidationSettings>` in `<TreeListSettings>`. Define the custom validator in the `ValidatorTemplate`. 
